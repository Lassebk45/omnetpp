...
const char *NedParser::getBuiltInDeclarations()
{
    return
        ...
        "    bool enableUtilization @mutable = default(false);\n"
        "    double utilizationSamplingDuration @mutable = default(5s) @unit(s); // packet error rate (PER)\n"
        "    double utilizationEmitRate @mutable = default(1s) @unit(s); // packet error rate (PER)\n"
        "}\n"
        "\n"
